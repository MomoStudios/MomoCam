...
def main():
    ...
    try:
        ...
        while True:
            ...
            if args.synchronous:
                child_result = []
                analyze_frame(resized, child_result, engine, labels, args.local)
            else:
                if not is_processing:
                    # kick off analysis in subprocess, if not currently analyzing
                    is_processing = True
                    next_child_result = []
                    child = multiprocessing.Process(target=analyze_frame, args=(resized, child_result, engine, labels, args.local))
                    child.start()
...
def analyze_frame(frame, boxes, engine, labels, local):
